...
\begin{tabular}{@{} L{4.2cm} C{2.6cm} C{2.4cm} C{2.4cm} C{2.2cm} @{}}
\toprule
\textbf{Configuration} & \textbf{Effort ratio} & \textbf{Pearson $r$} & \textbf{Final error} & \textbf{Energy} \\
\midrule
$w_2 = 1$ (as reported) & $1.91 \pm 0.15$ & $0.808$ & 0.1033 & 327\,048 \\
$\mathbf{w_2 = 5}$ & $\mathbf{1.43}$ & $\mathbf{0.911}$ & $\mathbf{0.1019}$ & 91\,810 \\
$w_2 = 20$ & $\mathbf{1.34}$ & $\mathbf{0.932}$ & 0.1249 & \textbf{38\,855} \\
\midrule
Conventional controller \emph{(floor)} & $1.33$ & $0.916$ & 0.199 & 102\,876 \\
\bottomrule
\end{tabular}
...
At $w_2 = 20$ the learned policy attains an effort ratio of \textbf{1.34} against
...
\subsection{And at moderate weighting there is no trade-off at all}

The result at $w_2 = 5$ was not anticipated. Both this chapter and
\cref{sec:whygap} expected a Pareto trade-off --- less wasted effort bought with
worse accuracy. Instead $w_2 = 5$ is \textbf{better on both axes} than the
reported configuration: final error $0.1019$ against $0.1033$, closest approach
$0.0690$ against $0.0729$, energy reduced by 72\,\%, effort ratio $1.91
\rightarrow 1.43$, and agreement $0.808 \rightarrow 0.911$.

The configuration this thesis reports as its principal result is therefore
\emph{strictly dominated} by one differing only in a single reward weight.
...
\textbf{The reported configuration is not the best one found.} $w_2 = 5$
dominates it. The results of \cref{sec:replication} are retained as the
five-seed characterisation of the configuration this work analysed in depth, and
this section is reported alongside them rather than replacing them, because the
$w_2$ variants were run at a single seed each and have not been replicated.
