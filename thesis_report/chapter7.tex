...
\midrule
0 & 0.1063 & 0.0740 & \textbf{1.71} & \textbf{0.865} & 0.04 \\
...
\midrule
\textbf{mean} & $0.1033$ & $0.0729$ & $1.914$ & $0.808$ & $0.076$ \\
\textbf{std}  & $0.0034$ & $0.0032$ & $0.150$ & $0.044$ & $0.075$ \\
\bottomrule
\end{tabular}
...
Final error varies by 3.3\,\% across five independent runs
($0.1033 \pm 0.0034$\,m) and closest approach by 4.4\,\%
($0.0729 \pm 0.0032$\,m). Every run ends closer than \SI{0.11}{\meter} and
reaches within \SI{0.076}{\meter} at some point; the blow-up rate is exactly zero
...
\midrule
Final error (m) & 0.199 & \textbf{0.106} \\
...
\textbf{Effort ratio} & \textbf{1.33} & 1.91 \\
...
Measured against \textbf{1.33}, the achievable floor demonstrated by a controller
that optimises effort explicitly, it uses about \textbf{44\,\% more effort than
necessary}. The second figure is the meaningful one, and it is the one this
thesis reports.

The qualification remains that 1.33 is itself one measurement from one controller
at one pair of gains, and a better conventional controller might lower it.
...
\begin{tabular}{@{} L{3.8cm} C{1.2cm} C{2.6cm} C{2.4cm} C{2.4cm} C{1.8cm} @{}}
\toprule
\textbf{Configuration} & \textbf{$n$} & \textbf{Effort ratio} & \textbf{Pearson $r$} & \textbf{Final error} & \textbf{Energy} \\
\midrule
$w_2 = 1$ & 5 & $1.91 \pm 0.15$ & $0.808 \pm 0.044$ & $0.1033 \pm 0.0034$ & 327\,048 \\
$\mathbf{w_2 = 5}$ & 4 & $\mathbf{1.42 \pm 0.10}$ & $\mathbf{0.917 \pm 0.018}$ & $0.1051 \pm 0.0042$ & 101\,835 \\
$w_2 = 20$ & 1 & $1.34$ & $0.932$ & 0.1249 & \textbf{38\,855} \\
\midrule
Conventional \emph{(floor)} & 1 & $1.33$ & $0.916$ & 0.199 & 102\,876 \\
\bottomrule
\end{tabular}
...
At $w_2 = 20$ --- a single seed, replication pending --- the learned policy attains an effort ratio of \textbf{1.34} against
the achievable floor of \textbf{1.33} demonstrated by a controller that solves
minimum-effort load sharing explicitly at every timestep. Agreement with the
classical solution rises to $r = 0.932$, exceeding the conventional controller's
own $0.916$, and the per-muscle RMSE falls to 3.0\,\% of mean $F_{\max}$ from
10.1\,\%.
...
\subsection{At moderate weighting the effort cost is close to free}

A first, single-seed measurement of $w_2 = 5$ suggested it was better on
\emph{both} axes than the reported configuration, and an earlier draft of this
section described that configuration as strictly dominated. Replication across
four seeds does not support the accuracy half of that claim, and it is withdrawn.

What replicates, and cleanly:

\begin{itemize}[leftmargin=1.5em, itemsep=2pt]
\item \textbf{Effort ratio $1.91 \pm 0.15 \rightarrow 1.42 \pm 0.10$}, a 26\,\%
      reduction. The two distributions do not overlap: every $w_2 = 5$ seed
      (1.34--1.56) sits below every $w_2 = 1$ seed (1.71--2.12).
\item \textbf{Agreement with the classical solution
      $0.808 \pm 0.044 \rightarrow 0.917 \pm 0.018$}, and notably tighter across
      seeds --- the load-sharing solution becomes both closer to the classical
      one and more consistent between runs.
\item \textbf{Energy reduced by a factor of 3.2}, from 327\,048 to
      $101\,835 \pm 9\,586$.
\end{itemize}

What does not replicate is the accuracy advantage. Final error is
$0.1051 \pm 0.0042$ against $0.1033 \pm 0.0034$ --- overlapping, and marginally
\emph{worse} rather than better. Closest approach is $0.0709 \pm 0.0053$ against
$0.0729 \pm 0.0032$, also overlapping. The single seed originally analysed was
the most favourable of the four.

The defensible statement is therefore that raising the effort weight buys a
substantial improvement in load-sharing fidelity \textbf{at no measurable cost in
reaching accuracy} --- which is a weaker claim than dominance, and still the most
useful result in this chapter.

This is the second occasion on which a single-seed result in this work proved
optimistic in the same direction (\cref{sec:replication} was the first). The
pattern is not coincidence: when one run is selected for detailed analysis
because it performed well, its favourable metrics are selected along with it.
...
\textbf{The reported configuration is not the best one found.} At $w_2 = 5$ the
load-sharing fidelity is markedly better for no measurable accuracy cost, over
four seeds. The results of \cref{sec:replication} are retained as the five-seed
...
44\,\% above the achievable floor of $1.33$ (\cref{sec:effortfloor}), with the
...
discrepancy toward the $1.33$ floor. This is the most informative single experiment remaining, it
