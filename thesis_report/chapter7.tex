...
\midrule
$w_2 = 1$  & 5 & $1.914 \pm 0.150$ & $0.808 \pm 0.044$ & $0.1033 \pm 0.0034$ & $0.0729$ & 327\,048 \\
$w_2 = 5$  & 5 & $1.406 \pm 0.092$ & $0.919 \pm 0.016$ & $0.1023 \pm 0.0074$ & $0.0681$ & 104\,598 \\
$\mathbf{w_2 = 20}$ & 5 & $\mathbf{1.304 \pm 0.025}$ & $\mathbf{0.943 \pm 0.008}$ & $0.1072 \pm 0.0148$ & $\mathbf{0.0672}$ & \textbf{37\,076} \\
\midrule
Conventional & 1 & $1.33$ & $0.916$ & 0.199 & --- & 102\,876 \\
\bottomrule
\end{tabular}
...
$1.304 \pm 0.025$, comparable to the $1.33$ measured for a controller that
...
\subsection{Load-sharing fidelity improves monotonically, at no accuracy cost}
...
\textbf{There is no measurable accuracy cost.} Final error is
$0.1033 \pm 0.0034$, $0.1023 \pm 0.0074$ and $0.1072 \pm 0.0148$ --- mutually
overlapping, with no ordering supported. Closest approach in fact \emph{improves}
monotonically, $0.0729 \rightarrow 0.0681 \rightarrow 0.0672$. The expectation
stated in \cref{sec:whygap}, that reduced effort would be bought with worse
accuracy, is not borne out over the range tested.
...
controller's $1.33$; the dotted line is the idealised optimum of $1.00$, which
...
\subsection{A comparison that must be stated carefully}

At $w_2 = 20$ the learned policy attains an effort ratio of
$1.304 \pm 0.025$, which is \emph{below} the $1.33$ measured for a conventional
controller that solves minimum-effort load sharing explicitly at every timestep
(\cref{sec:effortfloor}), and its agreement with the classical solution
($0.943$) exceeds the conventional controller's ($0.916$).

Taken at face value this says a learned policy distributes force more
economically than a controller which optimises that very quantity in closed form.
That is possible in principle --- the conventional controller optimises
instantaneously and greedily, whereas a learned policy can trade effort across
time, and \cref{sec:effortfloor} established that instantaneous optimisation does
not achieve $1.00$ in closed loop either.

\textbf{But the comparison rests on a weak reference and should not be pressed.}
The $1.33$ figure comes from a single controller at a single pair of gains
selected from a coarse twelve-point grid. A finer search was begun and not
completed. A better-tuned conventional controller may well fall below $1.30$, in
which case the ordering reverses. The defensible claim is that the two are
\emph{comparable}, not that the learned policy wins.
