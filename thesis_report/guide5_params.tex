...
\texttt{EVAL\_FREQ} & 25--50k & Learning-curve spacing. Monitoring only; five episodes per point is far too noisy to report. \\
\bottomrule
\end{longtable}
...
\texttt{gradient\_steps} & $-1$ / 8 & \textbf{The replay ratio control, and a trap.} \S\ref{sec:defect5}. \\
...
The fix is \texttt{gradient\_steps = -1}, which means ``as many updates as
transitions collected'' and therefore holds the ratio at $\approx 1$ for
\emph{any} \texttt{N\_ENVS} --- the bug cannot silently return if the environment
count changes.
