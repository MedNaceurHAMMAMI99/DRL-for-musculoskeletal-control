...
\begin{enumerate}
  \item \textbf{\textit{Jerk} minimisation}~\parencite{Flash1985}: trajectories
        minimise the derivative of acceleration, producing smooth bell-shaped
        velocity profiles characteristic of human pointing movements.
  \item \textbf{End-point variance minimisation}~\parencite{Harris1998}: neural
        control signals carry noise whose variance grows with signal magnitude,
        and the trajectory is selected to minimise the variance of the
        \emph{final} position. The model accounts for saccades and arm movements
        under one principle, reproduces the speed--accuracy trade-off of Fitts'
        law, and recovers the two-thirds power law relating path curvature to
        hand velocity in drawing.
...
\subsection{Muscle Synergies and Modular Organisation}
...
\end{equation}
This decomposition, obtained through non-negative matrix factorisation~(NMF), is
...
\begin{itemize}
  \item Static optimisation with muscle-stress
        minimisation~\parencite{Crowninshield1981}:
        $\min \sum_i (F_i^M / F_{0,i}^M)^2$.
  \item Forward simulation with dynamic optimisation of
...
% -----------------------------------------------------------
\section{Musculoskeletal Modelling}
...
\begin{table}[H]
  ...
  \begin{tabularx}{\textwidth}{@{} L{2.4cm} L{2.6cm} X c @{}}
    ...
    MyoSuite    & Open-source & Musculoskeletal + DRL; MyoHand 23~DOF / 39~muscles, MyoLegs 16~DOF / 80~muscles & \cite{Caggiano2022} \\
    dm\_control & Open-source & Standardised RL environments, based on \MuJoCo{} & \cite{Tassa2020} \\
...
\MuJoCo{}~\parencite{Todorov2012}, acquired by DeepMind in 2021 and subsequently
released freely, is the common choice for \DRL{} work: a fast solver, robust
contact handling, and native Hill-type muscle actuators. An earlier version of this
paragraph claimed ``up to one million simulation steps per second on a standard
CPU'', which was unsourced and is not what this model achieves. Measured on the
host machine, the nine-muscle arm of this thesis integrates at
\SI{6.4}{\micro\second} per physics step, about $157\,000$ steps per second.
...
per-step computational cost. Because the fibre--tendon equilibrium
...
simulation timestep, for every muscle}. This model integrates physics at
\SI{500}{\hertz} with the policy acting every tenth step
(\cref{sec:compute}), so at the measured mean of 31.2 iterations per muscle the
solve accounts for roughly $9 \times 31.2 \times 500 \approx 1.4 \times 10^5$
Newton iterations per simulated second --- a workload growing linearly with
muscle count and integration rate.

Measured on the host CPU over 950 queries, the full nine-muscle solve averages
\SI{2165}{\micro\second} at a mean of 31.2 iterations, or roughly
\SI{240}{\micro\second} per muscle (\cref{sec:compute_efficiency}). In an offline
simulation this is unremarkable; on an embedded target it is not free.

\begin{quote}
\small
This subsection originally continued: \emph{``this bottleneck motivates the
central hypothesis of the present work: a trained RL policy [\ldots] can produce
equally accurate muscle activation commands in microseconds, with all the
computation amortised into a one-time training phase.''} That hypothesis was
tested and it failed, so the framing has been removed rather than left standing
as motivation for a result the thesis goes on to withdraw.

The failure has a specific cause worth stating here, because it changes what this
subsection is evidence for. The Newton--Raphson cost above belongs to
\emph{forward simulation} --- inverting the fibre--tendon equilibrium to find the
force a muscle produces at a given length. That is not the calculation a
controller would need to replace. The controller's problem is \emph{load
sharing}: given a required joint torque, choose the nine muscle forces. Solved
directly, that problem costs \SI{23.9}{\micro\second}, against
\SI{288.8}{\micro\second} for a forward pass of the trained network
(\cref{sec:latencycaveat}). Comparing the network against the wrong baseline made
it look roughly seven times faster; compared against the right one it is about
twelve times slower.

What survives is narrower: the network's cost is fixed and branch-free regardless
of biomechanical state, where the iterative solver's depends on convergence.
Where a hard real-time bound matters more than mean latency, that property has
value --- but it is not a speed advantage, and this thesis no longer claims one.
\end{quote}
...
\begin{figure}[H]
  ...
  \caption{Complete diagram of the Hill muscle--tendon model. The pennation
           angle~$\alpha$ between the fibre and the line of action is shown at the
...
Maximum isometric force varies as a function of normalised muscle length according
...
the optimal fibre length at which the muscle produces its maximum isometric
force~$F_0^M$.
...
\begin{equation}
  ...
  \label{eq:fv}
\end{equation}
where $\vnorm = \dot{l}^M / (v_\text{max}\,l^M_\text{opt})$ is the normalised
...
The passive force rises exponentially once the fibre exceeds its optimal
length~\parencite{Zajac1989}, normalised so that it reaches $F_0^M$ at twice the
optimal length:
\begin{equation}
  F_\text{PEE}(\lnorm) =
  \begin{cases}
    F_0^M \dfrac{e^{\,k_\text{PE}(\lnorm - 1)} - 1}{e^{\,k_\text{PE}} - 1}
      & \lnorm > 1 \\[6pt]
    0 & \lnorm \leq 1
  \end{cases}
  \label{eq:pee}
\end{equation}
with $k_\text{PE} = 5.0$. This gives $F_\text{PEE} = 0.004\,F_0^M$ at
$\lnorm = 1.1$, $0.130\,F_0^M$ at $\lnorm = 1.6$, and exactly $F_0^M$ at
$\lnorm = 2.0$.

\subsubsection{Series Elastic Element (SEE) --- Tendon}

The tendon carries no load below its slack length and is quadratic in strain
above it:
\begin{equation}
  F_\text{SEE}(\varepsilon^T) =
  \begin{cases}
    F_0^M \, k_T \, (\varepsilon^T)^2 & \varepsilon^T > 0 \\
    0 & \varepsilon^T \leq 0
  \end{cases}
  \qquad
  \varepsilon^T = \frac{l^T - l^T_\text{slack}}{l^T_\text{slack}}
  \label{eq:see}
\end{equation}
...
\begin{quote}
\small
\textbf{Both of these equations replace incorrect ones.} Earlier versions of this
section gave the PEE as a \emph{quadratic} $F_0^M k_\text{PE}(\lnorm-1)^2$ and the
SEE as $F_0^M(\lnorm^T-1)^2 / (k_T + \lnorm^T - 1)$ with $k_T \approx 0.0475$.
Neither form is the one implemented. The forms above are those in
\texttt{biomechanics/hill\_model.py} and reproduced in
Appendix~\ref{app:newton}, and they are what every result in this thesis was
computed with; the chapter, not the code, was wrong. The discarded PEE expression
was also internally inconsistent, since $k_\text{PE} = 5.0$ in a quadratic gives
$1.8\,F_0^M$ at $\lnorm = 1.6$ rather than the $\approx F_0^M$ its own text
claimed.

The correction has no effect on any reported result. \Cref{sec:solvervalidation}
validates the solver against \MuJoCo{}'s internal muscle model numerically
(5.25\,\% NRMSE, $r = 0.989$), and that validation exercised the implemented
equations throughout.
\end{quote}

\subsubsection{Pennation Angle Dynamics}

The pennation angle $\alpha$ (angle between the muscle fibres and the tendon line of
action) varies with fibre length according to the constant-thickness
constraint~\parencite{Zajac1989} --- the fibre-sheet height $l^M \sin\alpha$ is
held fixed, which is the standard assumption and is not the same as constant
volume:
\begin{equation}
...
\subsubsection{Fibre-Tendon Equilibrium Equation and Total Force}
...
(fibre length), solved numerically at each simulation step
...
Reinforcement learning~(\RL{}) is formalised as a Markov Decision
...
$\pi : \mathcal{S} \to \mathcal{A}$ that maximises the expected discounted return:
\begin{equation}
...
\textcite{Lillicrap2016} introduce DDPG, which brought the deterministic
policy-gradient formulation into the deep-network setting for continuous action
spaces. The actor learns a deterministic policy
...
\subsubsection{PPO --- Proximal Policy Optimisation}

PPO~\parencite{Schulman2017} optimises a clipped objective function to prevent
...
worldwide competition on the control of a bipedal musculoskeletal model, in which
\textit{off-policy} methods with replay memory featured prominently among the
leading entries. \textcite{Song2021} extend this approach to modelling human locomotion
...
data and \DRL{} to learn physically realistic behaviours, demonstrating the value of
...
\textcite{Codol2023} propose MotorNet, a differentiable simulator whose reference
effector is a two-joint, six-muscle planar arm --- monoarticular flexor and
extensor pairs at shoulder and elbow plus a biarticular pair --- fully integrated
...
making policies robust to inter-individual variability and modelling errors.
...
algorithms. \textcite{Agarwal2021} formalised a set of good practices (bootstrap
confidence intervals, \textit{interquartile mean}, performance profiles) that are
now considered standard for any serious benchmarking effort.

\textbf{This work does not meet that standard, and says so rather than implying
otherwise.} An earlier version of this paragraph claimed that the experimental
protocol ``adopts these recommendations''. It does not: no bootstrap confidence
interval, interquartile mean or performance profile is reported anywhere in this
thesis, and no significance test is performed
(\cref{sec:stats_protocol}). What is reported is mean and standard deviation over
three to five seeds, with the explicit rule that differences smaller than the
observed seed spread are not treated as an ordering. The
\textcite{Henderson2018} warning is therefore taken seriously as a
\emph{constraint on what may be concluded}, which is the honest use of it, rather
than claimed as a methodology that was followed.
...
\begin{enumerate}[label=(\roman*)]
  \item \DRL{} studies on musculoskeletal models frequently adopt a simulator's
        built-in muscle actuator without stating the constitutive equations or
        validating them against an independent implementation.
  \item Where learned controllers are compared against classical biomechanics,
        the comparison is usually at the level of task performance rather than
        of the per-muscle forces themselves --- which is the level at which the
        redundancy problem actually lives.
  \item Comparative benchmarks that include a tuned classical controller as a
        baseline, rather than only other learned methods, are uncommon.
  \item The effect of Domain Randomization on the robustness of musculoskeletal
        policies, applied uniformly across compared algorithms, has been little
        studied systematically.
\end{enumerate}

This work addresses (i) through the independently implemented and validated
solver of \cref{sec:solvervalidation}, (ii) through the per-muscle load-sharing
comparison of \cref{ch:forces}, and (iii) through the tuned impedance controller
of \cref{sec:conventional}. Contribution~C3 of \cref{sec:contributions} is the
principal one; the remainder are secondary or negative results.

\begin{quote}
\small
Two further gaps were claimed in earlier versions and are withdrawn. One asserted
that \emph{``no study has directly benchmarked the per-step computational cost of
physics-based muscle control against RL policy inference''} --- an unsupported
priority claim, and one attached to a comparison this thesis went on to show was
posed against the wrong baseline (\cref{sec:latencycaveat}). The other claimed a
gap in \emph{``quantitative comparison to human EMG data''}, which this work does
not fill: no electromyographic or measured-force data enters it at any point, and
\cref{sec:synergies} declines to interpret its one comparison against published
human synergies for exactly that reason. A gap a thesis does not close does not
belong in its positioning.
\end{quote}
